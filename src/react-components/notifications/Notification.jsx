...
function Notification() {
  ...
  function hadleNotificationClick(id) {
    setUsers((prev) =>
      prev.map((user) => (user.id === id ? { ...user, isUnread: false } : user))
    );
  }
      ...
      users.map((user) => (
        <div key={user.id}>
          <button
            ...
            className={user.isUnread && "button--activated"}
          >
              ...
              <div className="notify-content">
                <div className="text">
                  ...
                  <span className="group">
                    {user.link && (
                      <a href={user.link.href}>{user.link.group}</a>
                    )}
                  </span>
                  <span className="post">
                    {user.link && <a href={user.link.href}>{user.link.post}</a>}
                  </span>
                  <span className={user.isUnread && "redSignal"}></span>
                </div>
                {/* text*/}
                <span className="time">{user.time}</span>
                {user.link && (
                  <a href={user.link.href} className={user.chess}>
                    {user.secondary_img}
                  </a>
                )}
